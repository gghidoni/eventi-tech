...
@php
    ...
    // Carichiamo una sola volta i metadati tag utili al rendering badge (preview + selezionati).
    $tagIds = collect($this->displayOptions)
        ->pluck('value')
...
<div
    ...
    class="las-relative las-w-full mt-4"
>
    @if ($name)
        @if ($this->multiple)
    ...
    <div class="las-relative las-w-full">
        ...
        <div
            ...
            las-transition-colors !border-gray-600"
            ...
            <div class="las-flex las-min-w-0 las-flex-1 las-flex-wrap las-items-center las-gap-1.5">
                @if ($this->multiple)
                    @foreach ($selectedOptions as $chip)
                        @php
                            ...
                            $chipTag = $tagsById->get((int) $chipValue);
                            $chipName = $chipTag?->name ?? $chipLabel;
                            $chipBadgeColor = $chipTag?->badge_color ?? '#1F2937';
                            $chipLabelColor = $chipTag?->label_color ?? '#FFFFFF';
                            $chipIcon = $chipTag?->icon;
                        @endphp

                        <span
                            class="las-inline-flex las-items-center las-gap-1 las-rounded-md las-px-2 las-py-0.5 text-xs font-anta"
                            data-selected="{{ $chipValue }}"
                            style="background-color: {{ $chipBadgeColor }}; color: {{ $chipLabelColor }};"
                        ...
                        >
                            @if ($selectedSlot ?? false)
                                ...
                                {{ $selectedSlot($chip) }}
                            @else
                                {{-- Badge tag come nella single page evento. --}}
                                @if ($chipIcon)
                                    <img src="https://cdn.simpleicons.org/{{ rawurlencode(strtolower(trim((string) $chipIcon))) }}/{{ ltrim($chipLabelColor, '#') }}"
                                        alt="" class="w-3 h-3 shrink-0" loading="lazy">
                                @endif
                                <span class="las-max-w-[150px] las-truncate">{{ $chipName }}</span>
                            @endif
                ...
                @else
                    ...
                    @if ($this->hasSelection)
                        ...
                        @else
                            <span class="las-flex-1 las-truncate text-sm text-white">{{ $selectedOptions[0]['label'] }}</span>
                        @endif
                    @else
                        @if ($this->searchable)
                        <input
                    ...
                    @foreach ($this->groupedOptions as $groupName => $groupOptions)
                        ...
                        @foreach ($groupOptions as $option)
                            @php
                                ...
                                $optionTag = $tagsById->get((int) $optionValue);
                                $optionLabel = $optionTag?->name ?? $option['label'];
                                $optionBadgeColor = $optionTag?->badge_color ?? '#1F2937';
                                $optionLabelColor = $optionTag?->label_color ?? '#FFFFFF';
                                $optionIcon = $optionTag?->icon;
                            ...
                            >
                                ...
                                @else
                                    ...
                                    <span class="inline-flex items-center gap-1 px-1 py-0.5 rounded-md text-xs font-anta"
                                        style="background-color: {{ $optionBadgeColor }}; color: {{ $optionLabelColor }};">
                                        @if ($optionIcon)
                                            <img src="https://cdn.simpleicons.org/{{ rawurlencode(strtolower(trim((string) $optionIcon))) }}/{{ ltrim($optionLabelColor, '#') }}"
                                                alt="" class="w-3 h-3 shrink-0" loading="lazy">
                                        @endif
