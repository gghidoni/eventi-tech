...
@php
    ...
    $selectedOptionsKey = array_search($selectedOptions[0]['value'], array_column($this->options, 'value'));
    $selectedOptions[0]['image'] = $this->options[$selectedOptionsKey]['image'];
    ...
    <div class="las-relative las-w-full">
        ...
        >
            ...
            >
                ...
                @else
                    {{-- Flat Options --}}
                    @forelse ($this->options as $index => $option)
                        @php
                            $optionValue = $option['value'];
                            $optionLabel = $option['label'];
                            $optionImage = $option['image'] ?? null;
