...
| P1 | Queue e notifiche non hanno una semantica operativa unica | Il bootstrap usa temporaneamente `QUEUE_CONNECTION=sync` per non lasciare job sospesi, ma Compose non definisce ancora un worker. Alcune mail sono sincrone, altre passano da job; `Event::booted()` dispatcha il job dalla lifecycle del model e le connessioni queue hanno `after_commit=false`. | Stabilire worker, retry, failed job, idempotenza e dispatch after-commit come parte del runtime, separando i side effect dai model. |
...
| P2 | Il ciclo di vita di Meilisearch non e definito | Bootstrap, healthcheck, sync iniziale e import seed sono ora espliciti, ma non esistono ancora policy operative di rebuild/freshness, recovery o fallback durante indisponibilita dell'indice. | Definire reindicizzazione, gestione degli errori e aspettative di consistenza tra PostgreSQL e indice. |
| P2 | Le decisioni architetturali non hanno ancora un registro strutturato | Versioni, quickstart e contratto ambiente sono stati riallineati. Restano pero decisioni importanti su consistenza, deployment e requisiti non funzionali distribuite tra note di audit e documenti narrativi, senza stato e conseguenze tracciabili come ADR. | Trasformare le invarianti architetturali residue in specifiche verificabili e decision record. |
